from env_v11_13_max_twice import *
    ...
    for epoch in range(n_epochs):
        ...
        if (epoch+1) % display_step == 0:
            ...
            for j in range(n_test):
                ...
                #    print("True")
                # print(action)
                test, scooter_list, unmet_demand, met_demand, departure_rate_list = study_area.take_action_test(action)
                unmet_demand_list = np.sum(unmet_demand)
                met_demand_list = np.sum(met_demand)
#                 gini_index_unmet_list = gini(unmet_demand)
#                 gini_index_met_list = gini(met_demand)
            for mn in range(nS):
...
#             scooter_list = scooter_scooter_list[-1]
            print("Epoch " + str(epoch+1) + ", Average reward, unmet demand, met demand, scooter num, scooter loc, wANNR, moran, gini, theil, EMD, centroid change, dispersion change, kl divergence = " + "{:.3f}, {:.3f}, {:.3f}, {:.3f}, {:.3f}, {:.3f}, {:.3f}, {:.3f}, {:.3f}, {:.3f}, {:.3f}, {:.3f}, {:.3f}".format(avg_reward[-1], avg_unmet_demand[-1], avg_met_demand[-1], avg_scooter_num[-1], avg_scooter_loc[-1], avg_wANNR_scooter[-1], avg_moran_scooter[-1], avg_gini_index_scooter[-1], avg_theil_scooter[-1], avg_EMD_scooter[-1], avg_centroid_scooter[-1], avg_dispersion_scooter[-1], avg_kl_scooter[-1]))
