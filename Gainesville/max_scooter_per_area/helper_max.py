...
def create_study_area(services, max_scooter_per_area, deploy_cost, \
                   docked_cost, N, no_scooter_penalty, pickup_reward, equity_penalty, rider_test_list, logistics_car_cost_per_mile):
    ...
    study_area = Operation(N, deploy_cost, docked_cost, no_scooter_penalty, pickup_reward, max_scooter_per_area, equity_penalty, rider_test_list, logistics_car_cost_per_mile)

    # Adding multiple service areas to the study area.
    for loc, geometry, departure_rate, arrival_rate in services:
        study_area.add_service(geometry, departure_rate, arrival_rate, loc, max_scooter_per_area)
...
def reinforce(study_area, nS, estimator_policy, estimator_value, n_epochs, n_iters, \
    ...
    for epoch in range(n_epochs):
        ...
        for i in range(n_iters):
            ...
            R, scooter_list, unmet_demand, met_demand, departure_rate_list = study_area.take_action(action)
        ...
        if (epoch+1) % display_step == 0:
            ...
            for j in range(n_test):
                ...
                #    print("True")
                # print(action)
                test[j], scooter_scooter_list[j], unmet_demand, met_demand, departure_rate_list = study_area.take_action_test(action)
                unmet_demand_list[j] = np.sum(unmet_demand)
                met_demand_list[j] = np.sum(met_demand)
                gini_index_unmet_list[j] = gini(unmet_demand)
                gini_index_met_list[j] = gini(met_demand)
                for mn in range(nS):
                    scooter_dist_list[mn] = scooter_scooter_list[j][mn].scooters_total
                    scooter_geometry_list[mn] = scooter_scooter_list[j][mn].geometry
                ...
                scooter_num_list[j] = np.sum(scooter_dist_list)
                scooter_loc_list[j] = np.sum(scooter_dist_list != 0)
                wANNR_scooter_list[j] = wANNR(scooter_gdf)
                moran_scooter_list[j] = moran(scooter_gdf)
                gini_index_scooter_list[j] = gini(scooter_gdf['scooters'])
                theil_scooter_list[j] = theil(scooter_gdf['scooters'])
                EMD_scooter_list[j] = EMD(scooter_gdf)
                centroid_scooter_list[j], dispersion_scooter_list[j] = centroid_dispersion(scooter_gdf)
                kl_scooter_list[j] = kl_divergence(scooter_gdf, departure_rate_list)
            
#             index = np.argsort(test)[-3]
#             avg_reward.append(test[index])
#             avg_unmet_demand.append(unmet_demand_list[index])
#             avg_met_demand.append(met_demand_list[index])
#             avg_scooter_num.append(scooter_num_list[index])
#             avg_scooter_loc.append(scooter_loc_list[index])
#             avg_gini_index_met.append(gini_index_met_list[index])
#             avg_gini_index_unmet.append(gini_index_unmet_list[index])
#             avg_wANNR_scooter.append(wANNR_scooter_list[index])
#             avg_moran_scooter.append(moran_scooter_list[index])
#             avg_gini_index_scooter.append(gini_index_scooter_list[index])
#             avg_theil_scooter.append(theil_scooter_list[index])
#             avg_EMD_scooter.append(EMD_scooter_list[index])
#             avg_centroid_scooter.append(centroid_scooter_list[index])
#             avg_dispersion_scooter.append(dispersion_scooter_list[index])
#             avg_kl_scooter.append(kl_scooter_list[index])
#             scooter_list = scooter_scooter_list[index]
            ...
            avg_kl_scooter.append(np.mean(kl_scooter_list))
            index = np.argsort(test)[-3]
            scooter_list = scooter_scooter_list[index]
            print("Epoch " + str(epoch+1) + ", Average reward, unmet demand, met demand, scooter num, scooter loc, gini index unmet, gini index met, wANNR, moran, gini, theil, EMD, centroid change, dispersion change, kl divergence = " + "{:.3f}, {:.3f}, {:.3f}, {:.3f}, {:.3f}, {:.3f}, {:.3f}, {:.3f}, {:.3f}, {:.3f}, {:.3f}, {:.3f}, {:.3f}, {:.3f}, {:.3f}".format(avg_reward[-1], avg_unmet_demand[-1], avg_met_demand[-1], avg_scooter_num[-1], avg_scooter_loc[-1], avg_gini_index_unmet[-1], avg_gini_index_met[-1], avg_wANNR_scooter[-1], avg_moran_scooter[-1], avg_gini_index_scooter[-1], avg_theil_scooter[-1], avg_EMD_scooter[-1], avg_centroid_scooter[-1], avg_dispersion_scooter[-1], avg_kl_scooter[-1]))

    return avg_reward, avg_unmet_demand, avg_met_demand, avg_scooter_num, avg_scooter_loc, avg_wANNR_scooter, avg_moran_scooter, avg_gini_index_scooter, avg_theil_scooter, avg_EMD_scooter, avg_centroid_scooter, avg_dispersion_scooter, avg_kl_scooter, scooter_list
