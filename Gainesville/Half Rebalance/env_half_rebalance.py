...
class Operation:
    ...
    """
    def __init__(self, N, deploy_cost, docked_cost, no_scooter_penalty, pickup_reward, max_scooter_per_area, equity_penalty, rider_test_list, logistics_car_cost_per_mile, logistics_car_capacity):
        self.N = N
    ...
    def move_forward(self):
        ...
        if len(neg_gdf) > 0:
            ...
            total_rebalance_amount = min(np.sum(neg_gdf['value']), np.sum(pos_gdf['value']))
            while len(neg_gdf) > 0 and len(pos_gdf) > 0 and min(np.sum(neg_gdf['value']), np.sum(pos_gdf['value'])) >= total_rebalance_amount/2:
        ...
        return reward_0, np.array(unmet_demand_list), np.array(met_demand_list), departure_rate_list, cost  # np.sum(np.array(unmet_demand_list))
    ...
    def take_action(self, n=0):
        ...
        if n > 0:
            if np.random.rand(1) >= 0.1:
                iii = 0
                for service in self.map:
                    if service.riders > service.scooters: # and service.riders > 1
                        self.scooter_deploy_list[iii], self.scooter_total_list[iii] = service.add_scooter()
                    elif service.riders < service.scooters: # 
